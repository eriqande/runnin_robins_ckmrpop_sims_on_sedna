### This version uses the updated sib-finding function
library(tidyverse)
library(hexbin)
library(rdist)
library(CKMRpop)
...
cohort_size <- 2000
SampleSize = 89 ## fixed number to subsample from each cohort
samp_frac <- 2*SampleSize/cohort_size  ## twice as large as target for subsampling
SPD$`number-of-years` <- 56  # run the sim forward for 56 years
samp_start_year <- 51
samp_stop_year <- 55
NReps = 100
...
ds = cbind(Pedigree2[sibs[share_dad,]$i1,][, c('sampleID', 'YearBorn')],
           Pedigree2[sibs[share_dad,]$i2,][, c('sampleID', 'YearBorn')])
...
Pegigree2 = as.data.frame(Pedigree2)
...
sum(BigGapFull,na.rm=T)  ## total full sibs 
t(NData)
t(WangData)
